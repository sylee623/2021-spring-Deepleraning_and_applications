...
def layer_2_learning(x1, x2, y):
    global lr, K
    np.random.seed(0)
    W = [np.random.rand(3,2), np.random.rand(1,3)]
    b = [np.random.rand(3,1), np.random.rand(1,1)]
    X = np.array([x1,x2])
    Y = np.array(y)

    A = [np.zeros((3,m)), np.zeros((1,m))]
    Z = [np.zeros((3,m)), np.zeros((1,m))]
    dA = [np.zeros((3,m)), np.zeros((1,m))]
    dZ = [np.zeros((3,m)), np.zeros((1,m))]
    db = [np.zeros((3,1)), np.zeros((1))]
    dW = [np.zeros((3,2)), np.zeros((1,3))]
